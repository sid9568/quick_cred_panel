class Superadmin::AepsCommissionsController < Superadmin::BaseController

  before_action :set_commission, only: [:edit, :update, :destroy]

  # LIST
  def index
    @commission_slabs = AepsCommissionSlabRange.order(:min_amount)
    p "===================commission_slabs"
  ...
  # NEW FORM
  def new
    @commission_slab = AepsCommissionSlabRange.new
    @schemes = Scheme.where(user_id: current_superadmin.id)
  end

  # CREATE
  def create
    @commission_slab = AepsCommissionSlabRange.new(commission_params.merge(from_role: current_superadmin.role.title, to_role: "admin"))
  ...
  def set_commission
    @commission_slab = AepsCommissionSlabRange.find(params[:id])
  end
