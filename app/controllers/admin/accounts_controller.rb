class Admin::AccountsController < Admin::BaseController
  layout "admin"
  before_action :require_admin_login

  def index
    @account_transaction = AccountTransaction.where(parent_id: current_admin_user.id, txn_type: "Credit").order(created_at: :desc)
  end

  def new
  end

  def add_credit
    @user = User.find_by(username: params[:username], phone_number: params[:phone_number])

    unless @user
      redirect_to admin_accounts_index_path, alert: "User not found"
      return
    end

    if params[:set_pin] == current_admin_user.set_pin
      amount = params[:amount].to_f
      wallet = @user.wallet || @user.create_wallet(balance: 0)

      current_admin_user.wallet()

      parent_wallet = Wallet.find_by(user_id: current_admin_user.id) # parent wallet object
      p "==============-------------parent_walletparent_wallet"
      parent_wallet.update!(balance: parent_wallet.balance.to_f - amount)

      txn_id = "TXN#{rand(100000..999999)}"

      ActiveRecord::Base.transaction do
        wallet.update!(balance: wallet.balance.to_f + amount)

        AccountTransaction.create!(
          txn_id: txn_id,
          mobile: params[:phone_number],
          amount: amount,
          reason: params[:reason],
          wallet_id: wallet.id,
          txn_type: params[:type],
          user_id: @user.id,
          status: "success",
          parent_id: current_admin_user.id
        )
      end

      redirect_to admin_accounts_index_path, notice: "credited successfully"
    else
      redirect_to admin_accounts_index_path, alert: "Invalid set pin"
    end
  end

  def debit_index
    @account_transaction = AccountTransaction.where(parent_id: current_admin_user.id, txn_type: "Debit").order(created_at: :desc)
  end

  def add_debit
    @user = User.find_by(username: params[:username])

    unless @user
      redirect_to admin_accounts_add_debit_path, alert: "User not found"
      return
    end

    if params[:set_pin] == current_admin_user.set_pin
      amount = params[:amount].to_f
      wallet = @user.wallet || @user.create_wallet(balance: 0) # ensure wallet exists

      balance = wallet.balance.to_f

      if amount > balance
        redirect_to admin_accounts_add_debit_path, alert: "Insufficient balance"
        return
      end

      parent_wallet = Wallet.find_by(user_id: current_admin_user.id) # parent wallet object
      p "==============-------------parent_walletparent_wallet"
      parent_wallet.update!(balance: parent_wallet.balance.to_f + amount)

      remaining_balance = balance - amount
      txn_id = "TXN#{rand(100000..999999)}"

      ActiveRecord::Base.transaction do
        wallet.update!(balance: remaining_balance)

        AccountTransaction.create!(
          txn_id: txn_id,
          mobile: params[:phone_number],
          amount: amount,
          reason: params[:reason],
          wallet_id: wallet.id,
          txn_type: params[:type] || "debit",
          user_id: @user.id,
          status: "success",
          parent_id: current_admin_user.id
        )
      end

      redirect_to admin_accounts_add_debit_path, notice: "Debited successfully"
    else
      redirect_to admin_accounts_add_debit_path, alert: "Invalid set pin"
    end
  end





end
